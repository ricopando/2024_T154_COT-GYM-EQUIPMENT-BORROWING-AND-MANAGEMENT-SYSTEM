...
const Form = ({ userDetails, borrowedItems, onClose }) => {
    ...
    const generatePDF = (preview = false) => {
        ...
        const footer = () => {
            ...
            doc.text(`Document Code: OSP-F-001          Revision No: 0            Issue No. 1           Issue Date: 4/23/2021             Page 1of  ${pageCount}`, pageWidth / 2, startY + 15, { align: 'center' });
        };
        ...
        const mainContent = () => {
            ...
            if (borrowedItems && borrowedItems.equipment) {
                let currentRowY = tableStartY + rowHeight;

                borrowedItems.equipment.forEach((item) => {
                    ...
                    const requiredHeight = Math.max(rowHeight, (nameLines.length * 3.5) + 4);

                    // Draw cells with adjusted height
                    rowData.forEach((cellData, cellIndex) => {
                        // Draw cell rectangle
                        doc.rect(currentX, currentRowY, columnWidths[cellIndex], requiredHeight);
                ...
                const remainingRows = 6 - borrowedItems.equipment.length;
            ...
            signatureColumns.forEach((header, index) => {
                doc.rect(signatureX, signatureTableStartY, signatureColumnWidths[index], signatureRowHeight);
                doc.setFont('Arial', 'normal');
                doc.setFontSize(10);
            ...
            signatureRows.forEach((row, rowIndex) => {
                ...
                row.forEach((cellContent, cellIndex) => {
                    doc.rect(signatureX, y, signatureColumnWidths[cellIndex], signatureRowHeight);
                    ...
                    if (cellContent) {
                        ...
                        if (cellIndex === 0) {
                            ...
                            if (!(rowIndex === 3 && cellIndex === 1) && !(rowIndex === 1 && cellIndex === 1)) {
                                const textWidth = doc.getTextWidth(cellContent);
                                const centerX = signatureX + (signatureColumnWidths[cellIndex] / 2) - (textWidth / 2);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg shadow-xl w-[90%] max-w-4xl max-h-[90vh] overflow-hidden">
                ...
                <div className="px-6 py-4 border-b">
                    <h2 className="text-xl font-semibold text-gray-800">Borrower's Slip Preview</h2>
                </div>
                ...
                <div className="px-6 py-4 border-t bg-gray-50 flex justify-end space-x-4">
                    ...
                    <button 
                        ...
                        className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400 transition-colors flex items-center"
