...
const EmptyState = () => (
  <div className="flex justify-center items-center min-h-[60vh] bg-gray-100">
    <div data-aos="fade-up" className="p-6 mt-1 text-center bg-white rounded shadow max-w-sm">
    ...
    <div className="flex-grow p-3">
      <div className="flex justify-between items-start mb-2">
        <div>
          ...
          <span className="text-sm text-blue-500 border border-blue-500 rounded px-2">{item.equipment?.category}</span>
        </div>
        <button
      ...
      <div className="flex gap-2 items-center bg-gray-50 p-2 rounded">
        ...
        <div>
          <p className="text-sm font-medium">From: {new Date(item.startDate).toLocaleDateString()}</p>
          <p className="text-sm font-medium">To: {new Date(item.endDate).toLocaleDateString()}</p>
        </div>
      </div>
    </div>
  </div>
...
const BorrowList = () => {
  ...
  const handleRemoveItem = async (itemId) => {
    ...
    try {
      if (updatedItems.length === 0) {
        await axios.delete(`http://localhost:8000/api/borrow-lists/${borrowList._id}`, {
          withCredentials: true
        });
        setBorrowList(null);
  ...
  const handleSubmitList = async () => {
    ...
    try {
      ...
      const createBorrowedItems = await axios.post('http://localhost:8000/api/borrow-items', {
  ...
  if (loading) {
    return (
      <div className="relative">
        ...
        <div className="text-left mt-5 mb-12 container mx-auto px-4">
          <h1 data-aos="fade-up" className="text-5xl font-extrabold tracking-tight">Borrow List</h1>
        </div>
        <LoadingState />
      </div>
    );
        ...
        <div className="container mx-auto px-4">
          <div className="w-full bg-white rounded shadow border border-gray-200 flex flex-col">
            <div className="p-2 border-b border-gray-200 flex justify-between items-center">
              <span className="text-sm text-blue-500 border border-blue-500 rounded px-2">Draft</span>
            </div>
              ...
              {borrowList.items.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2">
                  {borrowList.items.map((item) => (
                    <ItemCard key={item._id} item={item} onOpenDeleteDialog={handleOpenDeleteDialog} />
                  ))}
                </div>
              )}
            </div>
              ...
              {openConfirmDialog && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-40">
                  <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full transition-transform transform scale-95">
                    ...
                    <p className="text-gray-600 mb-4">Are you sure you want to submit this borrow list? This action cannot be undone.</p>
                    <div className="flex justify-end gap-2">
                      <button onClick={handleCloseConfirmDialog} className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300 transition">Cancel</button>
                      <button 
              ...
              {openDeleteDialog && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-40">
                  <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full transition-transform transform scale-95">
                    ...
                    <p className="text-gray-600 mb-4">Are you sure you want to delete this item from your borrow list? This action cannot be undone.</p>
                    <div className="flex justify-end gap-2">
                      <button onClick={handleCloseDeleteDialog} className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300 transition">Cancel</button>
                      <button 
