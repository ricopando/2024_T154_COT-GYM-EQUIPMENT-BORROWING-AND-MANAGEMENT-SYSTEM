...
const Addtransaction = () => {
    ...
    const handleRemoveItem = (itemId) => {
        setSelectedItems(prev => prev.filter(item => item.equipment._id !== itemId));
    };
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            const response = await axios.post('http://localhost:8000/api/borrow-items/user', {
                userId: selectedUser,
    ...
    const isDateDisabled = (date, itemId) => {
      ...
      const highlightWithRanges = (date) => {
        const isHighlighted = borrowedItems.some((borrowedItem) => {
          if (Array.isArray(borrowedItem.equipment)) {
            return borrowedItem.equipment.some((equipmentItem) => {
              if (equipmentItem.equipment?._id && (equipmentItem.status === "Pending" || equipmentItem.status === "Approved")) {
                                    ...
                                    {filteredEquipment.map(item => (
                                        <div key={item._id} className="border p-4 rounded">
                                                ...
                                                <div>
                                                    ...
                                                    <DatePicker
                                                        ...
                                                        onChange={(date) => {
                                                            ...
                                                            updatedItems[index] = { ...item, borrowDate: date };
                                                            setEquipmentItems(updatedItems);
                                                        }}
                                                        className="p-2 border rounded text-sm"
                                                        minDate={new Date()}
                                                        filterDate={(date) => !isDateDisabled(date, item._id)}
                                                        dayClassName={(date) => highlightWithRanges(date, item._id)}
                                                    />
                                                </div>
                                                <div>
                                                    ...
                                                    <DatePicker
                                                        ...
                                                        onChange={(date) => {
                                                            const updatedItems = [...equipmentItems];
                                                            const index = updatedItems.findIndex(i => i._id === item._id);
                                                            updatedItems[index] = { ...item, returnDate: date };
                                                            setEquipmentItems(updatedItems);
                                                        }}
                                                        className="p-2 border rounded text-sm"
                                            ...
                                            <button
                                                ...
                                                disabled={selectedItems.some(selected => selected.equipment._id === item._id) || !item.borrowDate || !item.returnDate}
                                            >
                                                Add
                                            </button>
                                        </div>
                            ...
                            <div className="overflow-y-auto flex-1">
                                <div className="grid gap-4 pr-2">
                                    {selectedItems.map(item => (
                                        <div key={item.equipment._id} className="border p-4 rounded flex justify-between items-center">
                                            <div>
                                                <h3 className="font-bold">{item.equipment.name}</h3>
                                                <p className="text-sm text-gray-600">
                                                    {new Date(item.borrowDate).toLocaleDateString()} - {new Date(item.returnDate).toLocaleDateString()}
                                                </p>
                                            </div>
                                            <button
