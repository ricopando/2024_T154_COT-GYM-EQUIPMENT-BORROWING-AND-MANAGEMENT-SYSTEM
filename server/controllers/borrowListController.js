...
const processBorrowList = async (req, res) => {
    try {
        ...
        borrowList.processedBy = req.user._id; // Assuming you have user info in request

        if (status === 'Approved') {
            // Create borrowed items for each item in the list
            const borrowedItems = borrowList.items.map(item => ({
                user: borrowList.user,
            ...
            await BorrowedItem.insertMany(borrowedItems);
    ...
    try {
        ...
        if (borrowList.status !== 'Draft') {
            return res.status(400).json({ 
                message: "Cannot delete borrow list that has been submitted or processed" 
            });
        }
