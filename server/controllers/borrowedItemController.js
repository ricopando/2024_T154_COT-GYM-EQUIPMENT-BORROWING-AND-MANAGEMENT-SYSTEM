...
const getBorrowedItems = async (req, res) => {
  ...
  try {
    const borrowedItems = await BorrowedItem.find()
      ...
      .populate(
        "items.equipment",
        "name serialNumber image model category createdAt status borrowDate returnDate availabilityStatus"
      );
    ...
    const itemsArray = borrowedItems.map((item) => ({
      ...
      equipment: item.items.map((i) => ({
        ...
        id: i._id,
        availabilityStatus: i.equipment ? i.equipment.availabilityStatus : null,
      })),
      status: item.status,
...
const returnBorrowedItem = async (req, res) => {
  ...
  try {
    ...
    if (borrowedItem.status === "Returned") {
      return res
        .status(400)
        .json({ message: "These items have already been returned" });
    }

    // Update statuses
    borrowedItem.status = "Returned";
    borrowedItem.items.forEach((item) => (item.status = "Returned"));
    await borrowedItem.save();

    // Update equipment availability after checking each item
    await Promise.all(
      borrowedItem.items.map(async (item) => {
        // Check if the equipment is still borrowed in other transactions
        const isStillBorrowed = await BorrowedItem.exists({
          _id: { $ne: borrowedItem._id }, // Exclude current borrowedItem
          "items.equipment": item.equipment,
          "items.status": { $in: ["Borrowed", "Pending", "Approved"] },
        });

        // Only update to 'Available' if not borrowed elsewhere
        if (!isStillBorrowed) {
  ...
  try {
    ...
    await Promise.all(
      equipmentIds.map((equipmentId) =>
        Equipment.findByIdAndUpdate(equipmentId, {
...
const deleteBorrowedItemFromArray = async (req, res) => {
  ...
  try {
    ...
    const itemIndex = borrowedItem.items.findIndex(
      (item) => item._id.toString() === itemId
    );
    if (itemIndex === -1) {
      return res
        .status(404)
        .json({ message: "Item not found in the borrowed item" });
    }
    ...
    if (borrowedItem.items.length === 0) {
      ...
      return res.status(200).json({
        message: "Borrowed item document deleted as it has no items left",
      });
    }
    ...
    res
      .status(500)
      .json({ message: "Failed to remove item from borrowed item", error });
  }
};
...
const updateItemStatusInArray = async (req, res) => {
  ...
  try {
    ...
    const allApproved = borrowedItem.items.every(
    ...
    res
      .status(500)
      .json({ message: "Failed to update item status in array", error });
  }
};
...
const returnItemInArray = async (req, res) => {
  ...
  try {
    ...
    // Check if the equipment is still borrowed in other transactions
    const isStillBorrowed = await BorrowedItem.exists({
      _id: { $ne: borrowedItemId }, // Exclude current borrowedItem
      "items.equipment": borrowedItem.items[itemIndex].equipment,
      "items.status": { $in: ["Borrowed", "Pending", "Approved"] },
    });
    ...
    if (!isStillBorrowed) {
      await Equipment.findByIdAndUpdate(
        borrowedItem.items[itemIndex].equipment,
        { availabilityStatus: "Available" }
      );
    }

    // Check if all items are returned
    const allReturned = borrowedItem.items.every(
...
const getEquipmentHistory = async (req, res) => {
  ...
  try {
    ...
    const history = await BorrowedItem.find({ "items.equipment": equipmentId })
      ...
      .populate(
        "items.equipment",
        "borrowDate returnDate status category name serialNumber model"
      );
...
const updateItemDates = async (req, res) => {
  ...
  try {
    ...
    borrowedItem.items[itemIndex].borrowDate = borrowDate;
    borrowedItem.items[itemIndex].returnDate = returnDate;

    await borrowedItem.save();
    res.status(200).json(borrowedItem);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Failed to update item dates", error });
  }
};

const createBorrowedItemForUser = async (req, res) => {
  if (!req.user) {
    return res.status(401).json({ message: "User not authenticated" });
  }
  try {
    ...
    // Create the BorrowedItem with all items
    const borrowedItem = await BorrowedItem.create({
      user: userId, // Use the selected user's ID instead of req.user._id
    ...
    // Update equipment availability status to 'Pending'
    await Promise.all(
      items.map((item) =>
        Equipment.findByIdAndUpdate(item.equipment, {
