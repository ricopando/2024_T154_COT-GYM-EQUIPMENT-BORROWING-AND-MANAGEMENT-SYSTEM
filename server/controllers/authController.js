...
const register = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: `${isAdmin ? "Admin" : "User"} registered`, password });
  } catch (error) {
...
const getAllUsers = async (req, res) => {
  try {
    const users = await userModel
      .find({ role: "User" })
      .select(
        "email displayName role _id inventory transaction borrowed report userManagement image addTransaction"
      );
...
const getAllAdmins = async (req, res) => {
  try {
    const users = await userModel
      .find({ role: "Admin" })
      .select(
        "email displayName image role _id inventory transaction borrowed report userManagement addTransaction"
      );
...
const toggleAccess = async (req, res) => {
  ...
  try {
    ...
    const updatedUser = await userModel
      ...
      .select(
        "email displayName role _id inventory transaction borrowed report userManagement addTransasaction"
      );
